...
int MineMapLabel[DIM][DIM]; // 게임판에 대한 정보를 저장할 2차원 배열
...
inline int& label(int x, int y, int MineMapMask[DIM][DIM]) { return MineMapLabel[y][x]; }
// 좌표(x, y)에 해당하는 MineMapLabel 배열의 값을 반환하는 함수
inline bool isValid(int x, int y) { return (x >= 0 && x < nx && y >= 0 && y < ny); }
// 좌표(x, y)가 지뢰인지 검사하는 함수
inline bool isBomb(int x, int y, int MineMapMask[DIM][DIM]) { return isValid(x, y) && label(x, y, MineMapMask) == Bomb; }
// 좌표(x, y)가 빈 칸인지 검사하는 함수
inline bool isEmpty(int x, int y, int MineMapMask[DIM][DIM]) { return isValid(x, y) && label(x, y, MineMapMask) == Empty; }

// 좌표(x, y)에 해당하는 지뢰를 깨는 함수
void dig(int x, int y, int MineMapMask[DIM][DIM]) {
    // 유효한 좌표인지, 이미 열린 칸인지 검사
    if (isValid(x, y) && mask(x, y, MineMapMask) != Open) {
        mask(x, y, MineMapMask) = Open; // 칸 열기
        // 좌표(x, y) 주변에 지뢰가 없다면 재귀 호출로 주변 칸들도 열기
        if (label(x, y, MineMapMask) == 0) {
            dig(x - 1, y - 1, MineMapMask);
            dig(x - 1, y, MineMapMask);
            dig(x - 1, y + 1, MineMapMask);
            dig(x, y - 1, MineMapMask);
            dig(x, y + 1, MineMapMask);
            dig(x + 1, y - 1, MineMapMask);
            dig(x + 1, y, MineMapMask);
            dig(x + 1, y + 1, MineMapMask);
...
void mark(int x, int y, int MineMapMask[DIM][DIM]) {
    if (isValid(x, y) && mask(x, y, MineMapMask) == Hide)
        mask(x, y, MineMapMask) = Flag; // 해당 칸에 깃발 표시
}
...
void print(int MineMapMask[DIM][DIM]) {
    system("cls");
    ...
    for (int y = 0; y < ny; y++) {
        ...
        for (int x = 0; x < nx; x++) {
            if (mask(x, y, MineMapMask) == Hide)    printf(" □");
                ...
                if (isBomb(x, y, MineMapMask)) printf(" ※");
                else if (isEmpty(x, y, MineMapMask)) printf("  ");
                else printf("%2d", label(x, y, MineMapMask));
            }
        }
        printf("\n");
    }
...
int countNbrBombs(int x, int y, int MineMapMask[DIM][DIM]) {
    int count = 0; // count 변수를 초기화
    // (x, y) 주변 3x3 칸을 검사
    for (int yy = y - 1; yy <= y + 1; yy++)
        for (int xx = x - 1; xx <= x + 1; xx++)
            // 검사하는 칸이 유효하고, 해당 칸에 지뢰가 있다면 count를 증가
            if (isValid(xx, yy) && label(xx, yy, MineMapMask) == Bomb)
                count++;
    return count; // count 반환
}

// MineMapMask를 초기화하는 함수
void init(int MineMapMask[DIM][DIM], int total = 9) {
    // srand를 호출하여 난수 생성
    ...
    for (int y = 0; y < ny; y++)
        for (int x = 0; x < nx; x++) {
            ...
            label(x, y, MineMapMask) = 0;
        }
    nBomb = total; // nBomb에 total 할당
    // nBomb개의 지뢰를 랜덤으로 생성
    for (int i = 0; i < nBomb; i++) {
        ...
        do {
            x = rand() % nx;
            y = rand() % ny;
        } while (label(x, y, MineMapMask) != Empty);
        // 해당 칸에 Bomb 레이블 부여
        label(x, y, MineMapMask) = Bomb;
    }

    // 각 빈 칸 주변에 있는 지뢰의 개수를 계산하여 label에 저장
    for (int y = 0; y < ny; y++)
        for (int x = 0; x < nx; x++)
            if (label(x, y, MineMapMask) == Empty)
                label(x, y, MineMapMask) = countNbrBombs(x, y, MineMapMask);
}
...
// 게임 종료 조건을 체크하는 함수
int checkDone(int MineMapMask[DIM][DIM]) {
    ...
    for (int y = 0; y < ny; y++) // 지뢰 맵의 모든 y좌표에 대해서
        for (int x = 0; x < nx; x++) { // 지뢰 맵의 모든 x좌표에 대해서
            ...
            if (mask(x, y, MineMapMask) != Open) count++; // count를 증가시킨다.
             // 해당 좌표가 지뢰이면 -1을 반환한다. (패배 조건)
            else if (isBomb(x, y, MineMapMask)) return -1;
        }
    // count와 총 지뢰 개수가 같다면, 모든 지뢰가 찾아졌으므로 승리한다.
    return (count == nBomb) ? 1 : 0;
}

void playMineSweeper(int total, int MineMapMask[DIM][DIM]) {
    int x, y, status;
    init(MineMapMask, total); // 지뢰 맵을 초기화한다.
    do {
        print(MineMapMask); // 현재 지뢰 맵의 상태를 출력한다.
        bool isBomb = getPos(x, y); // 사용자로부터 x, y 좌표를 입력받는다.
        if (isBomb) mark(x, y, MineMapMask); // 해당 좌표가 지뢰라면, 그 좌표를 깃발 상태로 변경한다.
        else dig(x, y, MineMapMask); // 해당 좌표를 열어본다.
        status = checkDone(MineMapMask); // 게임 종료 조건을 체크한다.
    } while (status == 0); // 승패 조건이 만족할 때까지 반복한다.
    print(MineMapMask); // 최종 지뢰 맵의 상태를 출력한다.
    if (status < 0)
        printf("\n실패: 지뢰 폭발!!!\n\n");  // 지뢰가 폭발하여 패배한 경우
    else
        printf("\n성공: 탐색 성공!!!\n\n"); // 모든 지뢰를 찾아서 승리한 경우
...
int main() {
    ...
    scanf("%d", &total);
    playMineSweeper(total, MineMapMask);
}
