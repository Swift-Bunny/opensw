...
/* Game 클래스 - JFrame 클래스를 상속 받았기에 현 Game 클래스 내에 있는 모든 클래스 멤버들은 JFrame 클래의 멤버들을 사용할수 있다. */
    ...
    public Game(){
        ...
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); //프레임 우측 상단 에 X 버튼의 기능을 정의
...
/* MyActionListener 클래스 - ActionListener 인터페이스 구현 , Game클래스 내에 또 하나의 클래스 이기 때문에 이런걸 내부 클래스 라고 하는데 장점으로는 바깥 Game클래스의 멤버들을 함께 공유 하며 쓸수 있다는 것이다. */
        public void actionPerformed(ActionEvent e) {
/* ActionListener 인터페이스 구현을 했기 때문에 actionPerformed 메소드 정의 , 이벤트매개변수로 e로 지정 */
            if(e.getSource() == bt) { //버튼이 눌렀을때에 밑 코드 동작
...
/* 힌트 라벨이 사용자 입력 수 보다 난수값이  낮을 때이기 때문에 낮다는는 문구를 힌트 라벨이 추가 */
            ...
            } else if (e.getSource() == clear_bt) { // 초기화 버튼을 눌렀을 때에 밑 코드 처리
                result_label.setVisible(true);
                clear_label.setVisible(true);
...
/*초기화로 정해진 난수값 확인 , game_count - 게임횟수 전위형 증감 */
    ...
    public static void main(String[] args) { //메인 메소드 정의
        new Game(); // Game클래스의 디폴드 생성자인 Game()를 자동적으로 위에서 처리 하게 되는원리.
    }
}